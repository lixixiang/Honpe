...
import com.honpe.lxx.app.ui.main.oa.ui.position9.adapter.InnerDetailAdapter;
import com.honpe.lxx.app.ui.main.oa.ui.position9.bean.CarMapInfoBean;
import com.honpe.lxx.app.ui.main.oa.ui.position9.bean.CarReturnBean;
import com.honpe.lxx.app.ui.main.oa.ui.position9.bean.CarlonLatBean;
import com.honpe.lxx.app.ui.main.oa.ui.position9.bean.InnerDetailBean;
import com.honpe.lxx.app.utils.DateUtil;
import com.honpe.lxx.app.utils.LatteLogger;
import com.honpe.lxx.app.utils.PointsUtil;
...
/**
 * @ProjectName: Honpe
 * @CreateDate: 2020/7/20 11:36
 * @Author: 李熙祥
 * @Description: java类作用描述 派车详情页设计
 */
public class DetailListActivity extends BaseActivity implements AMap.OnMapLoadedListener {
    ...
    public void initView() {
        ...
        if (TextUtils.isEmpty(RetTime) && !TextUtils.isEmpty(SetOutTime)) {
            curRetTime = YearMonthDayHourMinSS.format(DateUtil.setDate(YearMonthDayHourMin, curTime));
            curOutTime = YearMonthDayHourMinSS.format(DateUtil.setDate(YearMonthDayHourMin, SetOutTime));
            NewCurOutTime = DateUtil.addDateMinut(YearMonthDayHourMinSS, curOutTime, -8);
            NewCurRetTime = DateUtil.addDateMinut(YearMonthDayHourMinSS, curRetTime, -8);
        } else if (!TextUtils.isEmpty(RetTime) && !TextUtils.isEmpty(SetOutTime)) {
            curRetTime = YearMonthDayHourMinSS.format(DateUtil.setDate(YearMonthDayHourMin, RetTime)); //返厂时间
            curOutTime = YearMonthDayHourMinSS.format(DateUtil.setDate(YearMonthDayHourMin, SetOutTime));//出厂时间
            NewCurOutTime = DateUtil.addDateMinut(YearMonthDayHourMinSS, curOutTime, -8);
            NewCurRetTime = DateUtil.addDateMinut(YearMonthDayHourMinSS, curRetTime, -8);
        } else {
                ...
                .execute(new SimpleCallBack<String>() {
                    ...
                    public void onSuccess(String s) {
                        ...
                        for (CarMapInfoBean.DataBean dataBean : bean.getData()) {
                            ...
                            String mCarNo = StringUtil.replace(str, object);
                            if (CarNo.equals("粤BQ7J66")) {
                                ToastUtil.getInstance().showToast("此车未装有GPS");
    ...
    private void requestMap(CarReturnBean item, String strSetOutTime, String strReTime) {
        LatteLogger.d("requestMap",token + "   "+ item.getCarId() + "   "+strSetOutTime + "   "+ strReTime);
